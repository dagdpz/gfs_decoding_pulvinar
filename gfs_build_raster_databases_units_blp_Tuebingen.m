...
areaName = {'V1&2', 'V1', 'V2', 'V4'};
areaNum = {[1 2 3], 1, 2, 4};

n = 1;

% parameters(n).currArea = areaName;% char: 'pulvinar', 'V4', 'LGN'
% parameters(n).currAreaNum = areaNum;% numeric: 1 for pulvinar, 2 for LGN, 4 for V4
% parameters(n).eyemodi = [];% numeric vector: [0 1] for either eye configuration, 0 for 1-eye stimulation in the target period
% parameters(n).rep = 14;% 14
% parameters(n).fieldList = {'physdis', 'nodisappdat', 'subjdis', 'subjNodis'};% cell: {'CatchTargRemov', 'TargBotheyes', 'nocatch_disap', 'nocatch_nodisap'} - shrink the list if you need less
% parameters(n).disapList = {'disap', 'nodisap', 'disap', 'nodisap'};% cell: {'disap', 'nodisap', 'disap', 'nodisap'}
% parameters(n).conditionList = {'physical', 'physical', 'ambiguous', 'ambiguous'};% cell: {'physical', 'physical', 'ambiguous', 'ambiguous'}
% parameters(n).currSaveFolder = 'allAreas_ambiguous';% char: name of the folder to save
% parameters(n).field2decode = 'disap_nodisap'; % char: 'disap_nodisap', 'condition_disap', 'condition'
% parameters(n).excludeByUnitQual = 0;% numeric: 0 if you don't want to match blp and units and exclude blp by unit quality on the channel or 1 if you want
% n = n + 1;

% matching with units
for areaId = 1:length(areaName) % loop through brain areas I have
    
    parameters(n).currArea = areaName{areaId};% char: 'pulvinar', 'V4', 'LGN'
    parameters(n).currAreaNum = areaNum{areaId};% numeric: 1 for pulvinar, 2 for LGN, 4 for V4
    parameters(n).eyemodi = [0 1];% numeric vector: [0 1] for either eye configuration, 0 for 1-eye stimulation in the target period
    parameters(n).rep = 14;% 14
    parameters(n).fieldList = {'physdis', 'nodisappdat', 'subjdis', 'subjNodis'};% cell: {'CatchTargRemov', 'TargBotheyes', 'nocatch_disap', 'nocatch_nodisap'} - shrink the list if you need less
    parameters(n).disapList = {'disap', 'nodisap', 'disap', 'nodisap'};% cell: {'disap', 'nodisap', 'disap', 'nodisap'}
    parameters(n).conditionList = {'physical', 'physical', 'ambiguous', 'ambiguous'};% cell: {'physical', 'physical', 'ambiguous', 'ambiguous'}
    parameters(n).currSaveFolder = [areaName{areaId} '_both_cvsplits' num2str(parameters(n).rep)];% char: name of the folder to save
    parameters(n).field2decode = 'disap_nodisap'; % char: 'disap_nodisap', 'condition_disap', 'condition'
    parameters(n).excludeByUnitQual = 0;% numeric: 0 if you don't want to match blp and units and exclude blp by unit quality on the channel or 1 if you want
    n = n + 1;
    ...
    parameters(n).rep = 14;
    parameters(n).fieldList = {'subjdis', 'subjNodis'};
    parameters(n).disapList = {'disap', 'nodisap'};
    parameters(n).currSaveFolder = [areaName{areaId} '_ambiguous_cvsplits' num2str(parameters(n).rep)];% char: name of the folder to save
    parameters(n).conditionList = {'ambiguous', 'ambiguous'};
    parameters(n).field2decode = 'disap_nodisap';
    parameters(n).excludeByUnitQual = 0;
    n = n + 1;
    ...
    for wilcoxState = [0 1] % decode from neurons with significant / non-significant response to the target (Wilcoxon)
        ...
        parameters(n).disapList = {'disap', 'nodisap'};
        parameters(n).conditionList = {'ambiguous', 'ambiguous'};
        parameters(n).currSaveFolder = [areaName{areaId} '_ambiguous_response' num2str(wilcoxState)];
        parameters(n).field2decode = 'disap_nodisap';
...
%         parameters(n).conditionList = {'ambiguous', 'ambiguous'};
%         if MWU == 0
%             parameters(n).currSaveFolder = [areaName{areaId} '_noMatchUnits_ambiguous'];
%         elseif MWU == 1
...
%         parameters(n).eyemodi = 0;
%         parameters(n).rep = 14;
%         parameters(n).fieldList = {'CatchTargRemov_shifted', 'TargBotheyes_shifted', 'nocatch_disap_shifted', 'nocatch_nodisap_shifted'};
%         parameters(n).disapList = {'disap', 'nodisap', 'disap', 'nodisap'};
%         parameters(n).conditionList = {'physical', 'physical', 'ambiguous', 'ambiguous'};
%         if MWU == 0
%             parameters(n).currSaveFolder = [areaName{areaId} '_RTaligned_noMatchUnits_both'];
%         elseif MWU == 1
...
% make the file list of MUA data
muaDir = 'C:\Users\lvasileva\Documents\Luba\scripts\ndt_prep\GFS_Tuebingen\blp_data\mua\';
muaList = dir([muaDir '*.mat']);

% calculate background parameters for each neuron
disap_physical = zeros(386, 1);
nodisap_physical = zeros(386, 1);
diff_index = zeros(386, 1);

for muaNum = 1:length(muaList)
    
    load([muaList(muaNum).folder filesep muaList(muaNum).name], ...
        'physdis', 'nodisappdat', 'subjdis', 'subjNodis', 'all_data')
    
    disap_physical(muaNum) = mean2(physdis(501:800, :));
    
    nodisap_physical(muaNum) = mean2(nodisappdat(501:800, :));
    
    % divide data into two groups, calc d' equation
    halfData = size(all_data, 2)/2;
    halfData = floor(halfData);
    half1std = std(mean(all_data(501:800, 1:halfData), 1));
    half2std = std(mean(all_data(501:800, end-halfData+1:end), 1));
    
    diff_index(muaNum) = ...
        (half1std - half2std) ./ (half1std + half2std);
    
    clear physdis nodisappdat subjdis subjNodis all_data
...
for muaNum = 1:length(muaList)
    ...
    dt = [physdis nodisappdat subjdis subjNodis];
    
    target_response = mean(dt(851:1100, :), 1); % 50-300 ms after target onset (response)
    background = mean(dt(301:800, :), 1); % 300 ms before the target onset (background)
    
    [p_wilcox(muaNum), h_wilcox(muaNum)] = ranksum(target_response, background);
...
% data dir
data_list = {'delta', 'theta', 'alpha', 'beta', 'gamma1', 'gamma2', 'mua'};
% data_list = {'mua'};

for dataNum = 1:length(data_list)
    
    dataDir = ['C:\Users\lvasileva\Documents\Luba\scripts\ndt_prep\GFS_Tuebingen\blp_data\' data_list{dataNum} '\'];
    ...
    if sum(cellfun(@strcmp, fl_list, elec_info.sessionInfo_long)) ~= 386
        error('Figure out the correct file order')
    end
    
    for setNum = 1:length(parameters)
        ...
        currAreaId = ismember(elec_info.area, parameters(setNum).currAreaNum); % from pulvinar
        ...
        if ~isempty(parameters(setNum).wilcox_resp)
            if parameters(setNum).wilcox_resp == 1 % significant response to the target
                unitId = currAreaId & rep14' & currEyemodi & h_wilcox;
...
%                 constant_background' & below200' & equalFR';
        ...
        curr_info.sessionInfo_long = elec_info.sessionInfo_long(unitId);
%         curr_info.blp_elec = elec_info.blp_elec(unitId);
        curr_info.mua_elec = elec_info.mua_elec(unitId);
%         curr_info.unitqual = elec_info.unitqual(unitId);
        ...
        for flNum = 1:length(currFiles)
            ...
            raster_site_info.info_id = curr_info.sessionInfo_long{flNum};%dt.AddInfo.file;
            raster_site_info.channel = curr_info.mua_elec(flNum);
%             raster_site_info.eyemodi = dt.AddInfo.eyemodi;
            ...
            savename = [data_list{dataNum} '_' parameters(setNum).currArea '_' curr_info.sessionInfo_long{flNum} ...
                '_ch' num2str(curr_info.mua_elec(flNum)) '.mat'];
            
            disp(['Saving ' savename])
            fprintf('\n')
            save([saveDir savedir filesep savename], 'raster_data', 'raster_labels', 'raster_site_info')
