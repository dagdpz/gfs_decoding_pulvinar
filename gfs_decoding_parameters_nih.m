function [params] = gfs_decoding_parameters_nih
...
for dataTypeNum = 1%:length(data_list)
    ...
    % loop through all the areas (LGN, V4, pulvinar)
    for areaNum = 3:length(area_list)
        
        % repeat the same decoding in pulvinar several times, save results
        for repNum = 1:3
        
            params(n).mkdir1 = ['results_' area_list{areaNum} '_matchUnits_ambiguous_' data_list{dataTypeNum} '_cvsplits14_' num2str(repNum)];
            params(n).mkdir2 = [params(n).mkdir1 filesep 'shuff'];
