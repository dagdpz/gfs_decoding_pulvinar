...
sessionNames = cellfun(@(x) x(24:end-4), flList, 'Uniformoutput', false);
...
%     SInf = getSesInfo_sorted_lats_J_lnv(currSession);
    SInf = getSesInfo_fixgfs_04_lnv(currSession, 1);
    ...
    if strcmp(currSession, '200207_barney_gfs1-04') || ...% unequal number of elements
        ...
        % create a field with all the trials
        dt.SPKSEL.all_data = cat(3, dt.SPKSEL.TargOnly, dt.SPKSEL.TargRemov, dt.SPKSEL.disap, dt.SPKSEL.nodisap);
        
        dt.LFPSEL.all_data = cat(3, dt.LFPSEL.TargOnly, dt.LFPSEL.TargRemov, dt.LFPSEL.disap, dt.LFPSEL.nodisap);
        ...
        if length(SInf.spk_elec) ~= size(dt.LFPSEL.(fieldList{1}), 2)
            disp(currSession)
...
%         nocatch_nodisap_fake = randsample(dt.LFPSEL.RPL.nc_disap, size(dt.LFPSEL.nocatch_nodisap, 3), true); % ambiguous condition
        ...
        for chNum = 1:size(dt.SPKSEL.(fieldList{1}), 2)
