...
for areaId = 1:length(areaName) % loop through brain areas I have
    ...
    for MWU = [1 0] % create data matching and no-matching with units
        ...
        parameters(n).rep = 14;
        parameters(n).fieldList = {'TargOnly', 'TargRemov', 'disap', 'nodisap'};
        ...
        parameters(n).rep = 14;
        parameters(n).fieldList = {'disap', 'nodisap'};
...
%     n = n + 1;
    ...
    for wilcoxState = [0 1] % decode from neurons with significant / non-significant response to the target (Wilcoxon)
        ...
        parameters(n).disapList = {'disap', 'nodisap'};
        parameters(n).conditionList = {'ambiguous', 'ambiguous'};
        parameters(n).currSaveFolder = [areaName{areaId} '_response' num2str(wilcoxState)];
        parameters(n).field2decode = 'disap_nodisap';
...
spkDir = 'C:\Users\lvasileva\Documents\Luba\scripts\ndt_prep\GFS_noreport\blp_data\spk\';
spkList = dir([spkDir '*.mat']);

% calculate background parameters for each neuron
disap_physical = zeros(106, 1);
nodisap_physical = zeros(106, 1);
diff_index = zeros(106, 1);

for spkNum = 1:length(spkList)
    
    load([spkList(spkNum).folder filesep spkList(spkNum).name], ...
        'T_TargOnly', 'T_TargRemov', 'all_data')
    
    disap_physical(spkNum) = 1000/25*mean2(T_TargOnly(29:40, :)); % binsize is 25 ms for this dataset; from stable fixation 700 ms to target onset 1000 ms is 29 to 40
    
    nodisap_physical(spkNum) = 1000/25*mean2(T_TargRemov(29:40, :));
    
    % divide data into two groups, calc d' equation for the background (period of stable fixation)
    % stable fixation - within 701:1000ms for target-triggered data (target presentation is at 1000 ms)
    halfData = size(all_data, 2)/2;
    halfData = floor(halfData);
    half1std = std(mean(all_data(29:40, 1:halfData), 1));
    half2std = std(mean(all_data(29:40, end-halfData+1:end), 1));
    
    diff_index(spkNum) = ...
        (half1std - half2std) ./ (half1std + half2std);
    
    clear TargOnly TargRemov all_data
...
for spkNum = 1:length(spkList)
    ...
    load([spkList(spkNum).folder filesep spkList(spkNum).name], ...
        'T_TargOnly', 'T_TargRemov', 'T_disap', 'T_nodisap')
    
    dt = [T_TargOnly T_TargRemov T_disap T_nodisap];
    
    target_response = mean(dt(43:60, :), 1); % target onset @ 1000 ms for target-triggered data, and I want 50-500 ms after the target onset; with 25 ms bin it will be from 43 to 60
    background = mean(dt(29:40, :), 1); % 300 ms before the target onset (background)
    
    [p_wilcox(spkNum), h_wilcox(spkNum)] = ranksum(target_response, background);
    
    clear T_TargOnly T_TargRemov T_disap T_nodisap
    
end

% % figure out target-responsive neurons
% h_wilcox = elec_info.targetResponse;
...
for dataNum = 1:length(data_list)
    ...
    if sum(cellfun(@strcmp, fl_list, elec_info.sessionInfo)) ~= 106
    ...
    for setNum = 1:length(parameters)
        ...
        curr_info.blp_elec = elec_info.spk_elec(unitId);
        ...
        for flNum = 1:length(currFiles)
            ...
            raster_site_info.info_id = curr_info.sessionInfo{flNum};
...
%             raster_site_info.eyemodi = dt.AddInfo.eyemodi;
            ...
            savename = [data_list{dataNum} '_' parameters(setNum).currArea '_' curr_info.sessionInfo{flNum} ...
