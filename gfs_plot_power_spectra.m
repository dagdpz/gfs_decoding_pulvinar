...
data_dir = 'F:\Luba\scripts\lfp_power_spectra\';

savedir = 'F:\Luba\scripts\lfp_power_spectra_plots\';
...
for flNum = 1:length(flList)
    ...
    undScrId = find(flList(flNum).name == '_', 1, 'last');
    session_id = flList(flNum).name(1:undScrId(1)-1);
    
    SInf = getSesInfo_sorted_lats_J(session_id);
    
    chId = strfind(flList(flNum).name, 'ch');
    chNum_char = flList(flNum).name(chId+2:chId+3);
